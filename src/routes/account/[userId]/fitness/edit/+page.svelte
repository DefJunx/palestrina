<script lang="ts">
	...
	const { errors, enhance, form, message } = superForm(data.form, {
		dataType: 'json',
		onSubmit() {
			loading = true;
		},
...
<form method="post" class=" mt-8 w-full" use:enhance>
	<div class="border-primary flex flex-col space-y-8 border p-8">
		{#each $form.fitnessData as _, i}
			<div class="flex items-center gap-x-4">
				...
				<button disabled={loading} class="btn-icon mt-6" on:click={(e) => removeParameter(i)}>
					<Minus />
				</button>
			</div>
		{/each}
	...
	<div class="border-primary my-4 border p-8">
		<TipTap bind:content={$form.fitnessNotes} />
	</div>
